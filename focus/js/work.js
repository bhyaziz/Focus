...
async function gettasks(projectName) {
  ...
  // ✅ ADDED: Initialize the tasks array for this project if it doesn't exist
...
function updatebydb() {
  document.querySelectorAll(".project-list li").forEach((project) => {
    ...
    // ✅ CHANGED: Improved check to ensure projectTasks[projectName] exists and is an array
    console.log("Tasks for this project:", projectTasks[projectName] ? projectTasks[projectName].length : 0)

    // ✅ CHANGED: Added Array.isArray check to ensure it's an array before filtering
    if (projectTasks[projectName] && Array.isArray(projectTasks[projectName]) && projectTasks[projectName].length > 0) {
      const totalTasks = projectTasks[projectName].length
      console.log("Total tasks:", totalTasks)
      const completedTasks = projectTasks[projectName].filter((task) => task.completed).length
...
async function getprojects() {
  ...
  // ✅ ADDED: Create an array to hold all the task loading promises
  const taskLoadingPromises = []

  results.forEach((project) => {
    const projectName = project.get("project")
    console.log(projectName)
    projectsTable.push(projectName)
    addProjectbydb(projectName)

    // ✅ CHANGED: Store the promise returned by gettasks
    const taskPromise = gettasks(projectName)
    taskLoadingPromises.push(taskPromise)
  })

  // ✅ ADDED: Wait for all tasks to be loaded before updating the UI
  await Promise.all(taskLoadingPromises)

  // ✅ CHANGED: Now that all tasks are loaded, update the progress
  updatebydb()
}
...
function renameProject(button) {
  ...
  if (newName && newName.trim() !== "") {
    ...
    if (test){
      ...
      if (oldName===project){
        let index = projects.indexOf(oldName);
...
function removeProject(button) {
  ...
  undoMessage.innerHTML = `
        Your project "${projectName}" is deleted
  ...
  }, 1000) // Remove undo message after 10 seconds
}
...
function handleTimerFinishforfinshtask(taskId) {
  document.getElementById(`question-${taskId}`).style.display = "none"
}
function finishedTask(taskId) {
  ...
  timer.interval = setInterval(() => {
    if (timer.seconds > 0) {
      ...
      timer.isActive = false
      timer.timerFinished = true
      handleTimerFinishforfinshtask(taskId)
    }
